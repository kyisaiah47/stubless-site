...
check('source rendering', /SOURCES\.map/.test(read('src/components/Console.tsx')), 'source rows render on the page');
...
check('single source', (read('src/lib/product.ts').match(/https:\/\/stubless\.thecompound\.tech/g) || []).length === 1, 'host is declared in product vocabulary');
